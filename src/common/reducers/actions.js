...
export const isNewVersionAvailable = async () => {
  const { data: latestReleases } = await axios.get(
    'https://api.github.com/repos/Mini-Kun/RoyalLaunch/releases?per_page=10'
  );
...
export const checkForPortableUpdates = () => {
  return async (dispatch, getState) => {
    ...
    if (newVersion) {
      const baseAssetUrl = `https://github.com/Mini-Kun/RoyalLaunch/releases/download/${newVersion?.tag_name}`;
